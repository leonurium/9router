 ...
 * 3. Optional: DB_PRUNE_MAX_AGE_DAYS (default 7), GITHUB_GIST_MAX_SIZE_MB (default 15)
 */

const GITHUB_API = "https://api.github.com";

const DEFAULT_GIST_MAX_SIZE_MB = 15;
const GIST_MAX_SIZE_BYTES = (() => {
  const mb = parseInt(process.env.GITHUB_GIST_MAX_SIZE_MB || String(DEFAULT_GIST_MAX_SIZE_MB), 10);
  return (Number.isFinite(mb) && mb > 0 ? mb : DEFAULT_GIST_MAX_SIZE_MB) * 1024 * 1024;
})();
...
export async function uploadToGist(gistId, token, data, filename = "9router-backup.sqlite") {
  if (data.length > GIST_MAX_SIZE_BYTES) {
    console.warn(`[gist] skip: file too large (${data.length} bytes > ${GIST_MAX_SIZE_BYTES} for ${filename})`);
    return null;
  }
